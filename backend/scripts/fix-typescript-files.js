...
/**
 * Master script to fix all TypeScript files with issues
 */

console.log('Starting TypeScript files fix process...\n');

// Fix integration tests
console.log('=== FIXING INTEGRATION TESTS ===');
try {
  execSync('node scripts/fix-integration-tests.js', { stdio: 'inherit' });
  console.log('Integration tests fixed successfully!\n');
} catch (error) {
...
try {
  execSync('node scripts/fix-unit-tests.js', { stdio: 'inherit' });
  console.log('Unit tests fixed successfully!\n');
} catch (error) {
  console.error('Error fixing unit tests:', error.message);
}

// Fix email utility
console.log('=== FIXING EMAIL UTILITY ===');
try {
  execSync('node scripts/fix-email-utility.js', { stdio: 'inherit' });
  console.log('Email utility fixed successfully!\n');
} catch (error) {
  console.error('Error fixing email utility:', error.message);
}

// Fix test-app.ts
console.log('=== FIXING TEST APP ===');
try {
  const cleanTestAppContent = `import express, { Express, Request, Response, NextFunction } from 'express';
...
export const createTestApp = (): Express => {
  ...
  const fs = require('fs');
  const testAppPath = path.join(__dirname, '../src/tests/utils/test-app.ts');
  fs.writeFileSync(testAppPath, cleanTestAppContent);
  console.log('Test app file fixed successfully!\n');
} catch (error) {
  console.error('Error fixing test app file:', error.message);
}

// Run TypeScript check to see if we've fixed the issues
console.log('=== RUNNING TYPESCRIPT CHECK ===');
try {
  // Check specific Shopify service files individually
  const { readdirSync } = require('fs');
  const modulesDir = path.join(__dirname, '../src/modules/marketplaces/adapters/shopify/services');
  const serviceFiles = readdirSync(modulesDir)
  ...
  for (const file of serviceFiles) {
    ...
    execSync(`npx tsc "${file}" --noEmit`, { stdio: 'inherit' });
  }
  
  console.log('TypeScript check on Shopify adapter services passed successfully!\n');
} catch (error) {
  console.error('TypeScript check failed:', error.message);
}

console.log('TypeScript file fixes completed! There may still be some files that need manual fixes, but the major issues have been addressed.');